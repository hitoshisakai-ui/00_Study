...
async function createUser(
  ...
  token: string,
  payload: { userId: string; displayName?: string; email?: string; password?: string; role?: 'admin' | 'user' },
) {
  const response = await request.post(`${API_BASE}/users`, {
    ...
    data: {
      ...
      email: payload.email ?? `${payload.userId}@example.test`,
      password: payload.password ?? 'password123',
      role: payload.role ?? 'user',
    },
  });
  expect(response.status(), 'create user API status').toBe(201);
  return response.json();
}

async function disableUser(request: APIRequestContext, token: string, id: number) {
  const response = await request.patch(`${API_BASE}/users/${id}/disable`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  expect(response.ok(), 'disable user API status').toBeTruthy();
  return response.json();
}
...
test.describe('MemoPad viewpoint-table mapped Playwright tests', () => {
  ...
  test('TC-REFRESH-FILTER-001-01 / VP-REFRESH-FILTER-001 リフレッシュ後は一覧条件が初期状態に戻る', async ({ page, request }) => {
    const token = await loginByApi(request);
    const prefix = unique('TC-REFRESH-FILTER-001');
    const created = [];
    for (let i = 0; i < 11; i += 1) {
      created.push(await createMemo(request, token, {
        title: `${prefix}-${String(i).padStart(2, '0')}`,
        body: 'refresh filter',
        tags: 'refresh-tag',
      }));
    }
    await request.post(`${API_BASE}/todos/${created[0].id}/favorite`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    await openLoggedInPage(page, request);

    await page.getByLabel('検索').fill(prefix);
    await page.getByLabel('タグ').selectOption('refresh-tag');
    ...
    await expect(page.getByLabel('お気に入りのみ')).not.toBeChecked();
    await expect(page.getByLabel('並び替え')).toHaveValue('updatedAt');
    await expect(page.getByRole('button', { name: '降順' })).toHaveClass(/active/);
  ...
  test('TC-USER-DISABLE-007-01 / VP-USER-DISABLE-007 無効化後の既存セッションは保護APIで拒否される', async ({ request }) => {
    ...
    await disableUser(request, adminToken, created.user.id);
  ...
  test('TC-COMMON-UI-001-01 / VP-COMMON-UI-001 サーバーエラー時に指定メッセージを表示する', async ({ page, request }) => {
    await openLoggedInPage(page, request);
    ...
    await page.getByRole('button', { name: '新規作成' }).click();
    await page.getByLabel('タイトル').fill(unique('TC-COMMON-UI-001'));
    await page.getByLabel('本文').fill('サーバーエラー確認本文');
    await page.getByRole('button', { name: '登録' }).click();
    await expect(page.getByText('処理中にエラーが発生しました。時間をおいて再度お試しください。')).toBeVisible();
  });

  test('TC-COMMON-UI-002-01 / VP-COMMON-UI-002 通信エラー時に指定メッセージを表示する', async ({ page, request }) => {
    await openLoggedInPage(page, request);
    await page.route('**/api/todos**', async (route) => {
      await route.abort();
    });
    ...
    await page.reload();
    await expect(page.getByText('通信に失敗しました。ネットワーク接続を確認してください。')).toBeVisible();
  });

  test('TC-COMMON-API-006-01 / VP-COMMON-API-006 サーバーエラー時に500を返す', async ({ request }) => {
    const token = await loginByApi(request);
    const response = await request.post(`${API_BASE}/todos`, {
      headers: {
